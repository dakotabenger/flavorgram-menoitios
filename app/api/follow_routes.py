...
def create_like(followerUser, follwedUser):
    existingFollow = db.Follow.query.filter(Follow.followingId==follwedUser, Follow.followerId==followerUser).first()
    if (not existingFollow):
        new_follow = db.Follow(followingId=followedUser, followerId=followerUser)
